// Package tools is the MCP-style action edge: every outward capability a
// host system has lives behind a Tool implementation. Core never speaks SMTP,
// HTTP, or any provider protocol directly — that hygiene is what lets the
// universal gate (agentkit/gate) be the sole policy point.
//
// This is the action-edge core of the agentkit framework: the Tool interface,
// the Registry, and the per-call idempotency-key plumbing. Concrete tools
// (send-email, etc.) live in the consuming application and register at boot;
// the Tool interface is stable across them.
package tools
